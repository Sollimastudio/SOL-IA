...
1. No Supabase SQL Editor, execute
   `supabase/migrations/202607240200_install_solia_vault.sql`.
...
7. Configure `META_ALLOWED_USER_IDS` com o mesmo UUID antes de conectar Meta Ads.
8. Somente agora defina `VITE_SECURE_MEMORY_ENABLED=true` na Vercel e faca um novo deploy.

## Testes obrigatorios antes da producao

- Sem login, a consulta `solia_memories` deve retornar zero linhas ou erro de permissao.
...
- Uma nova captura deve gerar uma linha em `solia_memory_audit_logs`.
